...
export default async function SocialAssignmentsPage() {
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-4xl font-bold tracking-tight">사회 과제</h1>
              <p className="text-lg text-muted-foreground">
                {user?.role === 'student'
                  ? `사회 과제 목록 및 제출 현황 (총 과제: ${assignments?.length || 0}, 제출완료: ${totalSubmitted})`
                  : '사회 과제 목록 및 제출 현황'}
              </p>
            </div>
          </div>
          {user?.role === 'teacher' && (
            <AssignmentFormDialog subject="social" />
          )}
        </div>
      </div>
